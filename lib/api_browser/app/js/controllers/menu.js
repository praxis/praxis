app.controller("MenuCtrl", function($scope, $state, Documentation) {
  ...
  Documentation.getIndex().success(function(index) {
    ...
    numeral_versions = jQuery.grep($scope.versions, function( n, i ) { return (! isNaN(parseFloat(n)) );})
                             .sort(function(a,b) { return parseFloat(b) - parseFloat(a) } );
    $scope.selectedVersion = $state.params.version || numeral_versions[0] || $scope.versions[0];
