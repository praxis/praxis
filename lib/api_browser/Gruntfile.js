module.exports = function(grunt) {
  ...
  var userDocsPath = process.env.USER_DOCS_PATH,
      pluginPaths  = process.env.PLUGIN_PATHS.split(':'),
  ...
  grunt.registerMultiTask('merge_bower', function() {
    ...
    if (options.install) {
      ...
      var done = this.async();
      exec('bower install --force-latest --production --config.interactive=false --config.analytics=false', { cwd: '.' }, function(error, result) {
        if (error) {
