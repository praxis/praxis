module Praxis
  ...
  class ResponseTemplate
    ...
    def initialize(response_name, group: :default, &block)
      @name = response_name
      @group = group
      @block = block
    end
    ...
    def compile(action=nil, **args)
      ...
      if block.parameters.any? { |(type, name)| name == :media_type && type == :keyreq } && action
        unless args.has_key? :media_type
          ...
          unless media_type
            raise "Could not default :media_type argument for response template #{@name}. \
            Resource #{action.resource_definition} does not have an associated mediatype and none was passed"
          end
          args[:media_type] = media_type
        end
      end
      Praxis::ResponseDefinition.new(name, group: group, **args, &block)   
    end
